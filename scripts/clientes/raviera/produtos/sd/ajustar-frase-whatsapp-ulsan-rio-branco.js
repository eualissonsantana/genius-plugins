    ...
    window.addEventListener("load", () => {
        const changeWhatsappPhrase = (headerWhatsappItems) => {
            headerWhatsappItems.forEach(whatsappItem => {
                ...
                if(whatsappDataName == "Recepção - Vendas"){
                    modifiedWhatsappLink = `${whatsappLink.split("&")[0]}&text=Olá, vim através do site! Gostaria de mais informações sobre os carros.`;
                }
                else if(whatsappDataName == "Pós Vendas - Agendamentos"){
                    modifiedWhatsappLink = `${whatsappLink.split("&")[0]}&text=Olá, vim através do WhatsApp Site! Gostaria de agendar um serviço.`;
                }
                else if(whatsappDataName == "Leads - Vendas"){
                    modifiedWhatsappLink = `${whatsappLink.split("&")[0]}&text=Olá, vim através do site! Gostaria de mais informações sobre os carros.`;
                }
